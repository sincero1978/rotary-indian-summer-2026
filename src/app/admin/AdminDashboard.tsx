              ...
              <Bar dataKey="count" fill={CHART_BAR_FILL} radius={[4, 4, 0, 0]} maxBarSize={44}>
                <LabelList dataKey="count" position="top"
                  content={({ x, y, width, value }) => (
                    <text x={Number(x ?? 0) + Number(width ?? 0) / 2} y={Number(y ?? 0) - 4}
                      textAnchor="middle" fill={labelColor} fontSize={labelSz} fontWeight={600}>
                      {value}
                    </text>
                  )} />
              </Bar>
                  ...
                  <Bar dataKey="count" radius={[0, 4, 4, 0]} maxBarSize={30}>
                    ...
                    <LabelList dataKey="count" position="right"
                      content={({ x, y, width, height, value }) => (
                        <text x={Number(x ?? 0) + Number(width ?? 0) + 6} y={Number(y ?? 0) + Number(height ?? 0) / 2}
                          dominantBaseline="central" fill={labelColor} fontSize={labelSz} fontWeight={600}>
                          {value}
                        </text>
                      )} />
                  </Bar>
    ...
    <div className={`min-h-screen transition-colors duration-300 ${t("bg-[#1a2e24]", "bg-[#f0f4f2]")}`}>
      ...
      <header className={`border-b px-6 lg:px-10 py-3 flex items-center justify-between gap-4 ${t("bg-[#152619] border-white/8", "bg-white border-gray-200 shadow-sm")}`}>
        <div className="flex items-center gap-4">
          <Image src="/rally-logo-v2.png" alt="RIST 2026" width={1184} height={621} className="h-14 sm:h-20 lg:h-24 w-auto" />
